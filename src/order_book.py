...
class Format:
	...
	def __init__(self, data, start_from):
		self.data = data
	...
	def outputBook(self, symetric_book):
		...
		df_rows = df_rows[self.start_from:]
		idx = [datetime.fromtimestamp(x).time() for x in idx]
		idx = idx[self.start_from:]

		my_output = pd.DataFrame(df_rows, columns = colnames, index = idx)
		return my_output
	
	def splitBidsAsks(self):

		bids = self.data
		asks = self.data

		bids = bids.drop(columns=['Ask_1','Ask_1_Vol','Ask_1_Ord', 
								  'Ask_2','Ask_2_Vol','Ask_2_Ord',
								  'Ask_3','Ask_3_Vol','Ask_3_Ord',
								  'Ask_4','Ask_4_Vol','Ask_4_Ord',
								  'Ask_5','Ask_5_Vol','Ask_5_Ord'])
		
		asks = asks.drop(columns=['Bid_1','Bid_1_Vol','Bid_1_Ord', 
								  'Bid_2','Bid_2_Vol','Bid_2_Ord',
								  'Bid_4','Bid_4_Vol','Bid_3_Ord',
								  'Bid_3','Bid_3_Vol','Bid_4_Ord',
								  'Bid_5','Bid_5_Vol','Bid_5_Ord'])
