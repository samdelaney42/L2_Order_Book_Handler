...
class Book:
	...
	def cancelationOfExistingLimitOrder(self, event):
		...
		if (order_to_cancel is not None):
			...
			self.cancelations.append([event.time, order_to_cancel.price, shares_to_subtract_from_limit_total, order_to_cancel.direction])
		else:
	...
	def deletionOfExistingLimitOrder(self, event):
		...
		if order_to_delete is not None:
			...
			self.deletions.append([event.time, order_to_delete.price, order_to_delete.shares, order_to_delete.direction])
		else:
			self.logger.info("ID {} does not exist".format(event.order_id))
...
###############################
	...
	def formatBook(self, start_from, levels):
		...
		self.book = self.outputBook(symetric_book, start_from, levels)
		return self.book
	...
	def outputBook(self, symetric_book, start_from, levels):
		...
		idx = [datetime.fromtimestamp(x).time() for x in idx]
		idx = idx[start_from:]
		...
		#my_output = my_output.drop_duplicates()
		return my_output
	...
	def splitBidsAsks(self, formatted_book):
		...
		asks=asks.rename(columns={'index':'Time'})

		bids=bids.reset_index()
		bids=bids.rename(columns={'index':'Time'})
